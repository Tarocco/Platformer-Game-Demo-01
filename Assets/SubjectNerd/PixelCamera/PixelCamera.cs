...
namespace SubjectNerd.Utilities
{
	...
	public class PixelCamera : MonoBehaviour, ISerializationCallbackReceiver
	{
		[Serializable]
		...
		private void SetupCamera(CamSettings settings)
		{
            ...
            // Pixel render size
            int[] pixelRenderSize = GetRenderTextureSize(screenRenderSize, settings.aspect, zoomLevel);

            //Debug.Log($"screenRenderSize = {screenRenderSize}");
            //Debug.Log($"pixelRenderSize = {pixelRenderSize[0]}, {pixelRenderSize[1]}");
            ...
            CalculateQuad(screenRenderSize, pixelRenderSize, zoomLevel);
			...
			int[] actualRenderSize = GetRenderTextureSize(renderSize, Vector2.one, 1.0f);
		...
		/// </summary>
		/// <returns></returns>
		protected static int[] GetRenderTextureSize(Vector2 screen_size, Vector2 aspect, float zoom)
		{
			float width_f = Mathf.Abs(screen_size.x / aspect.x) / zoom;
			float height_f = Mathf.Abs(screen_size.y / aspect.y) / zoom;

            int width = Mathf.CeilToInt(width_f);
            int height = Mathf.CeilToInt(height_f);

            return new[] { width, height };
		}

		private void CalculateQuad(Vector2 screenRenderSize, int[] pixelRenderSize, float zoom)
		{
            Vector2 pixelSize = new Vector2(pixelRenderSize[0], pixelRenderSize[1]) * zoom;
            quadOffset = pixelSize - screenRenderSize;
			quadOffset /= 2f;
			quadOffset.x /= (float)Screen.width;
			quadOffset.y /= (float)Screen.height;
